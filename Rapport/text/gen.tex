...
\label{sub:Jeu de Nim}
  ...
  \label{subsub:Jeu de Nim complexe}
    ...
    Un jeu de Nim complexe est un ensemble de jeu de Nim simples. Ils ont été résolus indépendamment par Roland Sprague en 1935 et Patrick Grundy en 1939\cite{tsgE}.
    ...
    \textit{
     ...
     \begin{figure}[h]
        ...
        \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3.5cm, semithick]
          ...
          \node[state]          (21) [below left of=22]  {$2\ allumettes,\ 1\ allumettes$};
          \node[state]          (11) [below left of=21]  {$1\ allumettes,\ 1\ allumettes$};
          \node[state]          (2)  [below right of=22] {$2\ allumettes$};
          \node[state]          (1)  [below of=21]       {$1\ allumettes$};
...
\label{sub:Hackendot}
  ...
  La résolution proposé en 1979 par Josef Ulehla est un dérivé de la méthode de résolution des jeux de Nim complexes. En effet, dans son papier \cite{UleHack} Ulehla cherche simplement une fonction qui permettrai de donner une valeur de nimber a une situation de jeu.
...
\hypertarget{GenArbres}
  ...
  Pour répondre a ce problème, madame Selmi m'a fourni le travail d'un ancien étudiant\cite{SamWor} qui a utilisé uniquement le parcours préfixe et suffixe sur les arbres et a permis de gérer l’intégralité de ce problème.
